...
This initial edge reader intentionally serves only the active window. A request before
`active_window_start`, or a missing active-window value that would require the base
checkpoint, returns JSON-RPC `-32001 Archive unavailable` rather than an incorrect
Ethereum default. Completed-window catalogs and checkpoint fallback remain native
server features.
...
encoded, while an index object is limited to 2 MiB encoded; checked demo and real
measurement indexes are far below that cap, and oversize references fail before R2
I/O. Index parsing validates at most 65,536 entries, retains only up to 64
matching fingerprint candidates, and drops its local encoded-buffer copy before data
fetches. Pure-Rust `ruzstd` decoding keeps only borrowed run/key/value slices and clones
...
<https://fossil-r2-gateway.brian-t-bland.workers.dev/rpc>. See the
[live demo guide](../docs/live-demo.md) for copy-paste `cast rpc` checks, expected
outputs, the published commit and R2 inventory, and the sparse dataset's limitations.
It requires no credentials and exposes no write, delete, or list operation.
...
account, storage, cross-object index/data references, Zstd, and code lookup. The Worker
core is platform-neutral Rust (`src/core.rs`), so native tests execute the exact decoder
compiled into WASM rather than a second test implementation.
