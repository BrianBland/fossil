...
The first sealed package must be an exhaustive block-zero anchor containing every genesis account, nonzero slot and code blob; later anchors are not supported. Every subsequent package is a contiguous successor and contains only changes. Epochs cover at most 1,000 contiguous finalized blocks; a short epoch may be sealed for publication freshness. Each epoch is retained as the canonical source of its normalized post-state events and block metadata.
