...
5. writes one new active-window directory; after its 64th sealed epoch descriptor it
   builds one closing checkpoint partition-by-partition, emits bounded
   subshards/manifests, the completed
...
Readiness verifies only head and commit. Directories and state objects remain lazy.
The fixed-size head is read through a hard streaming bound. When fetched, the active
